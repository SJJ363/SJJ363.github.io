...
- **SEO artifacts** are generated by `scripts/seo.js` (runs last in the workflow):
  ...
  tracker and its statistics page, the glossary, the market pages,
...
3j. **`/funding-feed.xml` carries new rounds, and it is the second feed
   ...
   **The visible half is three placements, and two of them were an
   ...
   • A **sixth footer item needs no re-measuring**, unlike a sixth nav
     ...
     CSS and no `?v=` bump (rule 3c-i). It has since taken a seventh and
     an eighth (`/funding/statistics/` and `/about/`, rules 3c-viii and
     11) and runs to three lines at 360px, still without overflow —
     which is the property that makes this list, unlike the nav, safe
     to grow.
