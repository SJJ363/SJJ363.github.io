...
- **SEO artifacts** are generated by `scripts/seo.js` (runs last in the workflow):
  ...
  `feed.xml`, and the structured data injected into `index.html` /
  `companies.html`.
...
3h. **`/feed.xml` carries the brief archive, and syndicating the wire
   ...
   A **funding feed is the obvious second one** — new rounds are the
   site's other original dataset (rule 3c-i) and the thing an industry
