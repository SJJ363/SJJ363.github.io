...
- **Data** (`data/`) is regenerated by GitHub Actions (`.github/workflows/news.yml`)
  ...
  on the first refresh of the day (see rule 3b-ii); `funding-extract.js` and
  `profile.js` add Claude's per-article funding verdicts and per-company
  profiles to `companies-store.json`.
...
`funding-extract.js` and `profile.js` need a logged-in Claude CLI or
`CLAUDE_CODE_OAUTH_TOKEN`, so a local `seo.js` builds the tracker from whatever
verdicts are already committed in `companies-store.json` plus the regexes for
the rest, and renders company pages with only the profiles already committed —
...
`PROMPT_VERSION` rewrites ~1,300 profiles.
